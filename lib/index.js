...
export default function slackin ({
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    let { active, total } = slack.users
    if (!name) return res.send(404)
    let frame = dom("script src=https://www.google.com/recaptcha/api.js"),splash({ coc, path, name, org, channels, active, total, large, iframe: true, gcaptcha_sitekey })
    res.type('html')
    res.send(frame.toHTML())
  })
